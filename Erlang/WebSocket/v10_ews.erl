...
start() ->
    Pid = spawn(fun() -> start_listen() end),
    register(?MODULE, Pid),
    Pid.
...
accept_handle(ListenSock) ->
	...
	{ok, Sock} ->
	    ...
	    Pid = spawn(fun() -> accept_handle(ListenSock) end),
	    io:format("accept handle pid: ~p~n", [Pid]),
	    wait_msg(Sock);
	{error, _} ->
	    start()
    end.
...
loop(Sock, Data0) ->
	...
	{tcp, Sock, Data} ->
		...
		if 
		    Data0 == <<>> ->
			process_buffer(Data, FrameOpcode, Sock, <<>>),
	                loop(Sock, <<>>);
		    true ->
			BinData = list_to_binary(Data),
			NewData = <<Data0/binary, BinData/binary>>,
			process_buffer(NewData, FrameOpcode, Sock, <<>>),
	                loop(Sock, <<>>)
		end
	    catch
		throw: X ->
...
process_buffer(<< Fin:1, _RSV:3, Opcode:4, _Masked:1, PayloadLen:7, Rest/bits >> = Bin, FrameOpcode, Sock, Payload0) ->
	...
	(ByteSizeRest2 < RealPayloadLen) ->
	    io:format("package_rest_not_enough, continue next tcp frame.~n", []),
	    loop(Sock, Bin);
	true ->
	    ok
    end,

    << MaskPayload:RealPayloadLen/binary, _RealRest/bits >> = Rest2,
